...
Cross-task transport tested whether those dense maps represented a task-independent basis. SST-2→WildGuard recovered 28.7% of the median gap with 30.1% same-task improvement retention; WildGuard→SST-2 recovered 37.4% with 37.9% retention. Only 3/8 comparisons were substantial; shuffled maps passed 0/8.

The planned pooled-map test next applied its held-out BoolQ qualification gate. All four linear cross-model gaps were large and significant, but target-trained AUROC was only 0.639–0.646, below the prespecified 0.75 oracle threshold. BoolQ was therefore non-qualifying, and no alignment maps were fit.

Coordinate mismatch is sufficient and explains substantial natural failure, but neither sparse recovery nor dense-map transport is universal. Pooled transport remains unresolved; the next preregistered test needs a third task that first satisfies a task-independent decodability gate.
