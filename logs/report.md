...
Fresh holdouts and disjoint calibration addressed overlap. Both tasks qualified, but no pooled condition passed both gates: recovery was 95.3%/71.4%, and disjoint-minus-overlap AUROC changed by −0.0041/−0.0032.

Objective-aware maps then separated fitting compatibility from held-out generalization. SmolLM passed AG News under two selectors but failed MNLI; OLMo failed every held-out condition despite near-perfect fit-task probe-bank recovery. This supports task-conditioned recoverability, not a global canonical coordinate system. Next, test locked task-specific low-rank corrections.
