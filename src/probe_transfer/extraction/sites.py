...
ACTIVATION_SITES = frozenset({RESIDUAL_STREAM, MLP_INTERMEDIATE})


def activation_width(activations: dict[str, Any], model: dict[str, Any]) -> int:
    site = activations.get("site", RESIDUAL_STREAM)
    key = "hidden_size" if site == RESIDUAL_STREAM else "intermediate_size"
    width = model.get(key)
...
class ActivationCapture:
    ...
    def __enter__(self) -> Self:
        if self.site == MLP_INTERMEDIATE:
            ...
            for block_index in self.block_indices:
                ...
                projection = layers[block_index - 1].mlp.down_proj
    ...
    def selected(self, output: Any, attention_mask: torch.Tensor) -> list[torch.Tensor]:
        if self.site == RESIDUAL_STREAM:
            ...
            if missing:
                raise RuntimeError(
                    f"MLP activation hooks did not run for blocks: {sorted(missing)}"
                )
    ...
    def _hook(self, block_index: int):
        def capture(_module: Any, inputs: tuple[Any, ...]) -> None:
            if len(inputs) != 1 or not isinstance(inputs[0], torch.Tensor):
                raise RuntimeError("MLP down projection must receive one activation tensor.")
            if block_index in self._captured:
                raise RuntimeError(f"MLP block {block_index} ran more than once.")
...
def _decoder_layers(model: Any) -> Any:
    ...
    if layers is None:
        raise TypeError("MLP activation capture requires a Llama-family decoder.")
    return layers
