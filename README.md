...
Multi-task panels share one YAML: `--task` selects the evaluation dataset, and `--fit` selects an alignment condition. Panels support `align --panel`, which checks included-task compatibility before held-out evaluation. Set `execution.prepare_materials: true` to prepare included-task inputs automatically. Output uses `EXPERIMENT_OUTPUT_DIR` on the worker.
